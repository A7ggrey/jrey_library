...
<body style="background-color: ;">
	...
	<div id="main">
		<div>
			<input type="search" name="" id="searchInput" onkeyup="sFunction()" placeholder="Search Exam Title">
		</div>
...
require('./../requires/database-Config.php');
		...
		type = 'Revision C.A.T.S' AND level = '$my_Level' AND class = '$my_Class' AND subject = '$subj12' OR 
		type = 'Revision C.A.T.S' AND level = '$my_Level' AND class = '$my_Class' AND subject = '$subj13' OR 
		type = 'Revision C.A.T.S' AND level = '$my_Level' AND class = '$my_Class' AND subject = '$subj14' OR 
		type = 'Revision C.A.T.S' AND level = '$my_Level' AND class = '$my_Class' AND subject = '$subj15' 
		...
		while ($rows = mysqli_fetch_assoc($run2)) { ?>
			...
			<td><a href="download-docs.php?docs=<?php echo $rows['docs'] ?>" class="alert-link text-decoration-none" style="color: whitesmoke;"><button class="btn btn-primary"><img src="./../../../icons/file-earmark-pdf.svg">&nbsp;&nbsp;Download <?php echo $rows['subject']?> <?php echo $rows['class'];?> <?php echo $rows['type'];?></a></button></td>
		<?php
	}
