...
if (empty($_POST['email']) || empty($_POST['password'])) {
	...
	if ($stmt = $dbconnect->prepare('SELECT id, email, phonenumber, password FROM students WHERE email = ?')) {
		...
		if ($stmt->num_rows > 0) {
			$stmt->bind_result($id, $email, $phonenumber, $password);
			$stmt->fetch();
...
<body class="body"><b>
	...
	<div class="small" style="background-color: #aaaa55; opacity: 85%;">
		...
		<p class="margin-left">
			<input type="checkbox" name="" onclick="passwordS()" id="ck" class="form-check-input">&nbsp;&nbsp;&nbsp;&nbsp;
            <label class="form-label" id="sp">Show Password</label>
        </p>
		
		<p class="margin-left"><input type="submit" name="" id="submit" value="Login" class="btn btn-primary btn-submit input-others"></p>

		<p class="margin-left">
			<a href="./pages/forgot-password.php" class="alert-link text-decoration-none link-me">Forgot Password?</a>
		</p>
